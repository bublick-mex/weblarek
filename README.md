...
## Данные
В приложении используются интерфейсы и типы, описывающие предметные области товар и покупатель.
...
#### Интерфейс IBuyer
Описывает данные покупательня необходимые для оформления заказа
...
#### Типы
`export type TPayment = 'card' | 'cash' | ''` - тип способов оплаты
`export type TFormErrors = Partial<Record<keyof IBuyer, string>>` - тип ошибки заполнения формы пользователем

### Модели данных 

#### Класс CatalogModel
Отвечает за хранение массива данных всех товаров и товара выбранного для отдельного просмотра.
Конструктор:
...
#### Класс BuyerModel
Отвечает за хранение, изменение и валидацию контактных данных покупателя и способа оплаты.
Конструктор:
...
`validate(): TFormError` - валидация данных
